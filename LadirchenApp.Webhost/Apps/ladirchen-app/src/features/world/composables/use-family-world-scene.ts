...
export const useFamilyWorldScene = (props: FamilyWorldSceneProps, emit: FamilyWorldSceneEmit) => {
  ...
  const storageTargetZone = computed<HouseZoneId>(() => view.value === 'garden' ? 'garden' :
    selectedRoomView.value === 'all' || selectedRoomView.value === 'garden' ? 'living-room' : selectedRoomView.value);
