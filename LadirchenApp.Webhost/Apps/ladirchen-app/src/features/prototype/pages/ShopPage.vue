<template>
  <div class="page page-padding shop-page">
    ...
    <v-btn-toggle v-model="activeTab" class="shop-tabs mb-5" color="primary" mandatory rounded="lg">
      ...
      <v-btn value="house">Hauskatalog</v-btn>
    </v-btn-toggle>
    ...
    <template v-if="activeTab === 'family'">
      ...
      <div class="d-flex align-end justify-space-between mb-3">
        ...
        <v-btn v-if="store.viewerRole === 'guardian'" color="primary" prepend-icon="mdi-plus" rounded="lg" size="small" variant="tonal" @click="rewardDialog = true">Hinzufügen</v-btn>
      </div>
    ...
    <template v-else>
      <v-btn-toggle v-model="houseArea" class="area-tabs mb-4" color="primary" mandatory rounded="lg" variant="outlined">
        <v-btn value="inside" prepend-icon="mdi-sofa-outline">Innen</v-btn>
        <v-btn value="outside" prepend-icon="mdi-flower-outline">Draußen</v-btn>
      </v-btn-toggle>
...
watch(
  () => route.query.new,
  (value) => {
    if (value === '1' && store.viewerRole === 'guardian') {
...
.shop-tabs :deep(.v-btn),
.area-tabs :deep(.v-btn) {
  min-width: 0;
}
.redemption-policy {
