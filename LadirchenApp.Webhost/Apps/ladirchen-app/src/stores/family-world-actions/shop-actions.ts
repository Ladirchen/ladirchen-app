...
export const shopActions = {
  addShopReward(this: FamilyWorldStoreContext, input: NewShopReward) {
    ...
    this.notify(input.isVisible === false
      ? 'notifications.shop.savedHidden'
      : input.availableFrom ? 'notifications.shop.savedScheduled' : 'notifications.shop.savedVisible', input.availableFrom
      ? { title: input.title, date: input.availableFrom }
      : { title: input.title });
  },
